...
export async function getDeepseekUsage(apiKey = null, proxyOptions = null) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        plan: "DeepSeek",
        message: `DeepSeek balance API error (${response.status}).`,
      };
    }
    ...
    console.error("[DeepSeek Usage] Failed to fetch balance:", error);
    return { message: "DeepSeek usage request failed." };
  }
}
