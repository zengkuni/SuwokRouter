...
export async function handleNonStreamingResponse({ providerResponse, provider, model, sourceFormat, targetFormat, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, reqLogger, toolNameMap, customToolNames, trackDone, appendLog, reqTag, log, client, strippedParams }) {
  ...
  if (contentType.includes("text/event-stream")) {
    ...
    try {
      ...
      recordFailure(`Invalid JSON response from ${provider}`);
      appendLog({ status: `FAILED ${HTTP_STATUS.BAD_GATEWAY}` });
  ...
  saveRequestDetail(buildRequestDetail({
    ...
    status: "success"
  }, { endpoint: clientRawRequest?.endpoint || null, client, strippedParams }), clientRawRequest).catch(err => {
