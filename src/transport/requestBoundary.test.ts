...
describe("request boundary", () => {
  test("rewrites supported public aliases without losing query strings", () => {
    const request = rewritePublicAlias(
      new Request("http://localhost/v1/models?limit=5"),
    );
  ...
  test("handles double-v1 and responses aliases", () => {
    expect(
      new URL(
        rewritePublicAlias(new Request("http://localhost/v1/v1/models")).url,
      ).pathname,
    ).toBe("/api/v1/models");
    expect(
      new URL(rewritePublicAlias(new Request("http://localhost/codex")).url)
        .pathname,
    ).toBe("/api/v1/responses");
    expect(
      new URL(
        rewritePublicAlias(new Request("http://localhost/codex/compact")).url,
      ).pathname,
    ).toBe("/api/v1/responses/compact");
    expect(
      new URL(rewritePublicAlias(new Request("http://localhost/responses")).url)
        .pathname,
    ).toBe("/api/v1/responses");
    expect(
      new URL(
        rewritePublicAlias(new Request("http://localhost/responses/compact"))
          .url,
      ).pathname,
    ).toBe("/api/v1/responses/compact");
    expect(queryCanonicalPath("/codex/compact")).toBe(
      "/api/v1/responses/compact",
    );
    expect(queryCanonicalPath("/responses/compact")).toBe(
      "/api/v1/responses/compact",
    );
  });
  ...
  test("rejects QUERY for write-oriented public aliases", () => {
    ...
    const query = new Request("http://localhost/codex/compact", {
      method: "QUERY",
    });
  ...
  test("only normalizes read-only QUERY requests", async () => {
    ...
    const normalized = normalizeQueryRequest(
      new Request("http://localhost/api/providers/catalog?x=1", {
        method: "QUERY",
        headers: { "x-test": "yes" },
      }),
    );
  ...
  test("marks a host peer that arrives from the container gateway", () => {
    ...
    const local = new Request("http://localhost:1212/", {
  ...
  test("drops a spoofed host-peer marker from the wire", () => {
    const trust = { gateway: "172.17.0.1", hostAddress: "192.168.127.254" };
    const request = new Request("http://localhost:1212/", {
      headers: { [HOST_PEER_HEADER]: "1" },
    });
  ...
  test("downgrades h2c requests to ordinary HTTP semantics", () => {
    const request = new Request("http://localhost/", {
      headers: { upgrade: "h2c", "http2-settings": "abc" },
    });
  ...
  test("assigns bounded limits by request class", () => {
    expect(getRequestBodyLimitBytes("/api/v1/images/generations", "POST")).toBe(
      256 * 1024,
    );
    expect(getRequestBodyLimitBytes("/api/v1/images/edits", "POST")).toBe(
      10 * 1024 * 1024,
    );
    expect(getRequestBodyLimitBytes("/api/v1/chat/completions", "POST")).toBe(
      8 * 1024 * 1024,
    );
    expect(
      getRequestBodyLimitBytes("/api/settings/migrate-9router", "POST"),
    ).toBe(25 * 1024 * 1024);
    expect(getRequestBodyLimitBytes("/api/providers", "POST")).toBe(
      1024 * 1024,
    );
  ...
  test("weights large bodies without letting them bypass the concurrency budget", () => {
    ...
    expect(
      estimateRequestWorkUnits(new Request("http://localhost/v1/models"), 0),
    ).toBe(1);
  });
