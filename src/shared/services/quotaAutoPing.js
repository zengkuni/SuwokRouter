...
import { refreshAndUpdateCredentials } from "@/lib/usageRefresh";
...
export async function runQuotaAutoPingTick(deps = createDefaultDeps(), state = g) {
  ...
  try {
    ...
    for (const [provider, providerConfig] of Object.entries(C.providers)) {
      ...
      const enabledIds = Object.keys(enabledMap).filter((id) => enabledMap[id] === true);
      if (enabledIds.length === 0) continue;

      const opts = { provider, isActive: true, authType: "oauth", ids: enabledIds };
      const targets = await deps.getProviderConnections(opts);
