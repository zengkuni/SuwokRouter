...
function isDockerRuntime() {
  return process.env.SUWOKROUTER_RUNTIME_MODE === "docker"
...
export function installPackageVersion(version) {
  ...
  const spec = `${pkg.name || "suwokrouter"}@${version}`;
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once("close", (code) => {
      if (code === 0) {
        resolve({ version, packageName: pkg.name || "suwokrouter", manager: update.command, output });
      } else {
