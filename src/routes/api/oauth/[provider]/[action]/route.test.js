...
describe("OAuth route flow validation", () => {
  ...
  test("accepts Grok CLI device polling without a verifier", () => {
    expect(hasRequiredDevicePollFields("grok-cli", "device-code", null)).toBe(
      true,
    );
    expect(
      hasRequiredDevicePollFields("grok-build", "device-code", undefined),
    ).toBe(true);
  });

  test("keeps Qoder's custom device verifier requirement", () => {
    expect(hasRequiredDevicePollFields("qoder", "device-code", null)).toBe(
      false,
    );
    expect(
      hasRequiredDevicePollFields("qoder", "device-code", "verifier"),
    ).toBe(true);
  });
