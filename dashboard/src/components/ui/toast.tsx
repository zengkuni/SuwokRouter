        ...
        {toasts.map((toast) => {
          ...
          return (
            <Toast.Root
              key={toast.id}
              className={cn(
                "absolute z-[calc(9999-var(--toast-index))] h-(--toast-calc-height) w-full select-text rounded-lg border bg-[color-mix(in_srgb,var(--popover),var(--color-black)_calc(1%*max(0,var(--toast-index,0))))] not-dark:bg-clip-padding text-popover-foreground [transition:transform_.22s_cubic-bezier(.22,1,.36,1),opacity_.18s,height_.12s,background-color_.22s] data-expanded:bg-popover dark:bg-[color-mix(in_srgb,var(--popover),var(--color-black)_calc(6%*max(0,var(--toast-index,0))))] dark:data-expanded:bg-popover",
              ...
              toast={toast}
            >
              <Toast.Content className="pointer-events-auto flex max-h-[min(14rem,calc(100vh-2*var(--toast-inset)))] min-w-0 items-center gap-3 overflow-y-auto overflow-x-hidden px-3.5 py-3 text-sm transition-opacity duration-180 data-behind:not-data-expanded:pointer-events-none data-behind:opacity-0 data-expanded:opacity-100">
                {Icon && (
                  <span
                    className="flex size-7 shrink-0 items-center justify-center rounded-full bg-muted/70 [&>svg]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0"
                ...
                <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                  <Toast.Title
                    className="select-text cursor-text font-medium leading-snug break-words [overflow-wrap:anywhere]"
                    data-slot="toast-title"
                  />
                  <Toast.Description
                    className="text-muted-foreground leading-snug break-words [overflow-wrap:anywhere]"
                ...
                <Toast.Close
                  ...
                  className="flex size-7 shrink-0 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted/70 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
...
function showToast(type: "success" | "error" | "info" | "warning", title: ToastInput): void {
  toastManager.add({ type, title });
}

type ToastFn = {
  (message: ToastInput): void;
...
function toastBase(message: ToastInput): void {
  showToast("info", message);
}
toastBase.success = (message: ToastInput) => showToast("success", message);
toastBase.error = (message: ToastInput) => showToast("error", message);
toastBase.info = (message: ToastInput) => showToast("info", message);
toastBase.warning = (message: ToastInput) => showToast("warning", message);
