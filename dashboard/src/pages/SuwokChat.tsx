...
const AgentActivityView = memo(function AgentActivityView({ activities }: { activities?: AgentToolActivity[] }) {
  ...
  return (
    <div className="mb-2 space-y-1.5" aria-label="Suwok Router Assistant activity">
          ...
          <div key={activity.id} className="rounded-lg border border-border/70 bg-surface/45 px-2.5 py-1.5 text-[11px]">
            ...
            {imageSources.length ? (
              <div className="mt-2 flex flex-wrap gap-2">
                {imageSources.map((src, index) => <img key={`${activity.id}-${index}`} src={src} alt="Generated by Suwok Router Assistant" className="max-h-64 max-w-full rounded-lg border border-border object-contain" />)}
              </div>
            ) : null}
          </div>
        );
...
export default function SuwokChat() {
  const navigate = useNavigate();
  const [workspace] = useState(() => {
    try {
      return parseSuwokChatWorkspace(window.localStorage.getItem(SUWOK_CHAT_WORKSPACE_STORAGE));
    } catch {
      return null;
    }
  });
  ...
  useEffect(() => {
    ...
    const nextModel = reconcileSuwokChatModel(model || storedModel, modelsQ.data);
    setModel(nextModel);
  }, [modelsQ.data, model]);
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = window.setTimeout(() => {
      const raw = serializeSuwokChatWorkspace({ model, systemPrompt, messages });
      if (raw) {
        try { window.localStorage.setItem(SUWOK_CHAT_WORKSPACE_STORAGE, raw); } catch {                                     }
      }
  ...
  function clearConversation() {
    ...
    try { window.localStorage.removeItem(SUWOK_CHAT_WORKSPACE_STORAGE); } catch {                               }
    setMessages([]);
    setInlineError("");
  ...
  async function streamReply(
    ...
    if (enableTools) {
      requestBody.tools = SUWOK_AGENT_TOOLS;
    ...
    for (let round = 0; round < maxToolRounds; round += 1) {
      ...
      for (const call of toolCalls) {
        ...
        try {
          ...
          toolResult = await executeSuwokTool(call.function.name, args, { apiKey, approve: requestAgentApproval });
                  ...
                  <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="mb-3 rounded-xl border border-amber-300/30 bg-amber-300/[0.06] p-3" role="dialog" aria-label="Approve JavaScript execution">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-[11px] leading-5 text-muted-foreground">Suwok Router Assistant requested a diagnostic script. It runs locally with a reduced environment and a 10-second timeout.</p>
                ...
                {messages.length === 0 ? (
                  <div className="flex h-full min-h-[30vh] flex-col items-center justify-center gap-2 px-6 text-center">
                    <span className="grid size-12 place-items-center rounded-xl border border-border bg-surface text-muted-foreground">
                      <img src="/logo/suwokchat.svg" alt="" width="24" height="24" className="size-6 object-contain" />
                    </span>
