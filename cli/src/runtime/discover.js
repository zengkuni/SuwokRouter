...
function defaultDataDir() {
  if (
    ...
    !(process.platform === "win32" && /^\//.test(process.env.DATA_DIR))
  ) {
...
function inspectDirectory(dir, manifest = null) {
  ...
  const localManifest =
    manifest || readJson(path.join(resolved, INSTALL_MANIFEST));
  if (localManifest?.mode === "bundled") throw new Error(LEGACY_BUNDLED_ERROR);

  const composeFile =
    localManifest?.composeFile && fileExists(localManifest.composeFile)
      ? localManifest.composeFile
      : fileExists(path.join(resolved, "docker-compose.yml"))
        ? path.join(resolved, "docker-compose.yml")
        : null;
  const sourceFile = fileExists(path.join(resolved, "src", "server.ts"))
  ...
  const mode =
    localManifest?.mode ||
    (composeFile ? "docker" : binaryFile && !sourceFile ? "binary" : "native");
  const manifestDataDir = localManifest?.dataDir;
  const dataDir =
    manifestDataDir && !String(manifestDataDir).startsWith("docker-volume:")
      ? manifestDataDir
      : defaultDataDir();
  return {
    mode,
    ...
    dockerVolume:
      manifestDataDir && String(manifestDataDir).startsWith("docker-volume:")
        ...
        : null,
    port: Number(localManifest?.port) || 1212,
...
function discoverInstall({ dir, cliDirectory } = {}) {
  ...
  const repositoryRoot = path.resolve(
    cliDirectory || path.join(__dirname, "..", "..", ".."),
  );
